...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log('📨 Background: Any message received:', message);
  if (message.type === 'TEST_BACKGROUND') {
    console.log('✅ Background script is working! Test message received:', message);
    sendResponse({ ok: true, message: 'Background script is working!' });
    return true;
  }
  // Return true to allow other listeners to handle the message
  return true;
});

// Simple test function to verify background script is loaded
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'SIMPLE_TEST') {
    console.log('🧪 Simple test message received in background script');
...
// Set to 'mock' for testing or when Chrome Prompt API is not available
...
// Check system requirements for Chrome Prompt API
async function checkSystemRequirements() {
  try {
    console.log('🔍 Checking system requirements for Chrome Prompt API...');
    ...
    if (availability === 'unavailable') {
      console.error('❌ Chrome Prompt API is not available on this device');
      console.error('💡 This could be due to:');
      console.error('   - Unsupported operating system (requires Windows 10+, macOS 13+, Linux, or ChromeOS)');
      console.error('   - Insufficient storage (requires 22GB free space)');
      console.error('   - Insufficient RAM/CPU (requires 16GB RAM + 4 cores for CPU mode)');
      console.error('   - Insufficient VRAM (requires >4GB VRAM for GPU mode)');
      console.error('   - Metered connection (requires unmetered connection for initial download)');
      return false;
    }
    
    if (availability === 'downloadable' || availability === 'downloading') {
      console.log('📥 Language model is downloading...');
      console.log('💡 This may take some time on first use. The model will be downloaded automatically.');
    }
...
async function initializeLanguageModel() {
  try {
    console.log('🔄 Initializing Chrome Prompt API...');
    ...
    // Get model parameters (optional, for debugging)
    try {
      ...
      console.log('⚠️ Could not get model parameters:', paramError.message);
    }
    
    // Create a new session with download progress monitoring
    ...
    // Verify session is working by checking usage
    console.log(`📊 Session created - Usage: ${languageModelSession.inputUsage}/${languageModelSession.inputQuota}`);
...
async function explainTermWithAPI(term, pageContext = '', userField = '', userExperience = '') {
  try {
    console.log('🤖 explainTermWithAPI called with:', { term, userField, userExperience, hasContext: !!pageContext });
    
    // Check availability first (this is required by the documentation)
    const availability = await LanguageModel.availability();
    ...
    if (availability === 'downloadable') {
      console.log('📥 Model is downloadable - user interaction required for download');
      // The model will be downloaded when we create the session (user interaction already happened via button click)
    }
    ...
    if (availability === 'downloading') {
      ...
      console.log('📊 Availability after wait:', newAvailability);
    }
    
    // Initialize session if not already done
    if (!languageModelSession) {
      console.log('🔄 Language model session not found, initializing...');
...
ADVANCED: [professional explanation within 150 words, including definition, relevance, components, example, and related terms]`;
    ...
    // Check session usage before making the call
    console.log(`📊 Session usage: ${languageModelSession.inputUsage}/${languageModelSession.inputQuota}`);
    
    console.log('🔄 Sending prompt to Language Model...');
    
    // Use the correct prompt format according to Chrome Prompt API documentation
    const response = await languageModelSession.prompt(prompt);
    console.log('✅ Received response from Language Model:', response?.substring(0, 200) + '...');
    ...
    console.log('📊 Parsed explanations:', {
      beginnerFound: !!beginnerMatch,
      advancedFound: !!advancedMatch,
      beginnerLength: beginnerMatch ? beginnerMatch[1].length : 0,
      advancedLength: advancedMatch ? advancedMatch[1].length : 0
    });
    ...
    console.log('🔄 Falling back to mock explanation due to API failure');
    // Fallback to mock data on API failure
...
async function explainTermMock(term, pageContext = '', userField = '', userExperience = '') {
  ...
  // Provide more detailed mock explanations
  const beginnerExplanation = `"${term}" is a key concept${fieldNote} that helps you understand the bigger picture. Think of it as a building block - once you grasp this idea, other related concepts will make more sense. In simple terms, it's about how things work together${contextNote}. For example, imagine it like learning the alphabet before reading words - this concept helps you understand more complex ideas later.`;
  
  const advancedExplanation = `"${term}" is a sophisticated concept${fieldNote} that involves multiple interconnected elements and principles. It represents a nuanced understanding of how different components interact within a larger system${contextNote}. This concept is particularly relevant because it demonstrates the complex relationships between various factors, showing how seemingly separate elements can influence each other. Understanding this concept requires recognizing patterns, analyzing relationships, and applying critical thinking to see how it fits into the broader context of the field.`;
...
// Message handling
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log('📨 Background: Raw message listener triggered');
    ...
    switch (message.type) {
      ...
      case 'GET_EXPLANATION':
        console.log('🤖 Background: GET_EXPLANATION received', {
          ...
          hasPageContext: !!message.pageContext
        });
        console.log('🤖 Background: Full GET_EXPLANATION message:', message);
        ...
        try {
          ...
          console.log('🤖 Background: Explanation generated successfully', {
            beginner: explanation.beginner?.substring(0, 100) + '...',
            advanced: explanation.advanced?.substring(0, 100) + '...',
            beginnerLength: explanation.beginner?.length || 0,
            advancedLength: explanation.advanced?.length || 0
          });
          ...
          console.log('🤖 Background: Sending successful response', response);
          console.log('🤖 Background: Response details:', {
            ok: response.ok,
            hasBeginner: !!response.beginner,
            hasAdvanced: !!response.advanced,
            beginnerLength: response.beginner?.length || 0,
            advancedLength: response.advanced?.length || 0
          });
          sendResponse(response);
        } catch (error) {
          console.error('🤖 Background: Error in GET_EXPLANATION:', error);
          const errorResponse = { 
            ok: false, 
            error: error.message,
            beginner: `Error getting explanation for "${message.term}": ${error.message}`,
            advanced: `Error getting explanation for "${message.term}": ${error.message}`
          };
      ...
      case 'DETECT_FIELD':
        // Auto-field detection functionality has been removed
        sendResponse({ ok: false, error: 'Field detection has been disabled' });
        break;
        ...
        try {
          ...
          if (tab) {
            ...
            try {
              ...
              console.error('❌ Background: Failed to open side panel:', sidePanelError);
            }
            ...
            const pageData = await chrome.storage.local.get(['pageText']);
            const pageContext = pageData.pageText || '';

            // Get real explanation using the existing explainTerm function
            const explanation = await explainTerm(message.term, pageContext, userField, userExperience);
            ...
            // Notify panel to update its display
            chrome.runtime.sendMessage({
              type: 'TERM_EXPLANATION_READY',
              term: message.term,
