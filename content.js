...
function createInlineExplanationDialog() {
  ...
  dialog.innerHTML = `
    <div class="hatrick-dialog-content">
      <div class="hatrick-dialog-actions">
        <button class="hatrick-action-btn" id="ask-hatrick" title="Ask Hatrick">
          <span class="hatrick-action-text">Ask Hatrick</span>
        </button>
  ...
  if (!document.getElementById('hatrick-inline-dialog-styles')) {
    ...
    style.textContent = `
      ...
      #ask-hatrick .hatrick-action-text {
        color: #007bff;
      }
      
      #ask-hatrick:hover .hatrick-action-text {
        color: #0056b3;
      }
...
function setupInlineDialogEventListeners(dialog) {
  ...
  const askBtn = dialog.querySelector('#ask-hatrick');
  ...
  if (askBtn) {
    askBtn.addEventListener('click', () => {
      console.log('🔗 Ask Hatrick clicked');
  ...
  if (termBtn) {
    termBtn.addEventListener('click', async () => {
      ...
      if (selectedText) {
        console.log('🔗 Sending showTermExplanation message to background');
        // Send message to background script to show term explanation in panel
        chrome.runtime.sendMessage({
          type: 'showTermExplanation',
          term: selectedText.trim()
        }, (response) => {
          console.log('🔗 Background response for term explanation:', response);
        });
        // Hide dialog but keep selection for processing
  ...
  if (notesBtn) {
    notesBtn.addEventListener('click', () => {
      ...
      if (selectedText) {
        console.log('🔗 Sending addToNotes message to background');
        // Send message to panel to add selected text to notes
        chrome.runtime.sendMessage({
          type: 'addToNotes',
          text: selectedText.trim()
        }, (response) => {
          console.log('🔗 Background response for add to notes:', response);
        });
        // Hide dialog
        hideInlineExplanationDialog();
      } else {
...
// Auto-detect field from page content
...
async function autoDetectField() {
  try {
    const url = window.location.href;
    
    // Check if we already have a cached result
    const cachedField = await getCachedFieldDetection(url);
    if (cachedField) {
      console.log('🎯 Using cached field detection:', cachedField);
      return cachedField;
    }
    
    // Get page content
    const pageResult = await chrome.storage.local.get(['pageText']);
    const pageContent = pageResult.pageText || '';
    
    if (!pageContent || pageContent.length < 100) {
      console.log('❌ Insufficient page content for field detection');
      return {
        category: 'General',
        subfield: null,
        fullPath: 'General',
    ...
    console.log('🤖 Requesting field detection from background script...');
    const response = await chrome.runtime.sendMessage({
      type: 'DETECT_FIELD',
    ...
    if (response && response.ok) {
      console.log('✅ Field detected:', response);
      
      // Cache the result
      await cacheFieldDetection(url, response);
      
      // Update user settings with detected field
      await chrome.storage.local.set({
        userField: response.displayField || response.subfield || response.category, // Use displayField (subfield takes priority)
        autoDetectedField: true, // Flag to indicate this was auto-detected
        fieldDetails: response // Store full field details
      });
      
      // Notify panel about field detection completion
      chrome.runtime.sendMessage({ 
        type: 'FIELD_DETECTION_COMPLETED', 
        fieldDetails: response 
      });
      
      return response;
    } else {
      console.log('❌ Field detection failed, using fallback');
      return {
        category: 'General',
        subfield: null,
        fullPath: 'General',
    ...
    console.error('❌ Auto field detection error:', error);
    return {
      category: 'General',
      subfield: null,
      fullPath: 'General',
...
// Helper functions for caching (content script version)
async function getCachedFieldDetection(url) {
  try {
...
async function cacheFieldDetection(url, fieldResult) {
  try {
    const result = await chrome.storage.local.get(['fieldDetectionCache']);
    const cache = result.fieldDetectionCache || {};
    cache[url] = fieldResult;
    await chrome.storage.local.set({ fieldDetectionCache: cache });
  } catch (error) {
    console.error('❌ Failed to cache field detection:', error);
  }
}

// Auto-detect field when page loads
async function initializeFieldDetection() {
  try {
    // Wait a bit for page content to be fully loaded
    // DISABLED: Auto-detection temporarily turned off
    // setTimeout(async () => {
    //   const detectedField = await autoDetectField();
    //   console.log('🎯 Field detection completed:', detectedField);
    // }, 2000); // 2-second delay to ensure content is loaded
  } catch (error) {
    console.error('❌ Field detection initialization failed:', error);
  }
}

// Initialize field detection when content script loads
initializeFieldDetection();
