...
const elements = {
  ...
  diveInVocabPocketHeader: document.getElementById('vocab-pocket-header'),
  diveInVocabPocketToggle: document.getElementById('vocab-pocket-toggle'),
  diveInVocabPocketContent: document.getElementById('vocab-pocket-content'),
...
// Handle Term Explanation Toggle click - REMOVED (toggle button removed)

// Handle Vocab Pocket Toggle click
function handleVocabPocketToggle() {
  const isCollapsed = elements.diveInVocabPocketContent.classList.contains('collapsed');
  
  if (isCollapsed) {
    // Expand vocab pocket
    elements.diveInVocabPocketContent.classList.remove('collapsed');
    elements.diveInVocabPocketToggle.classList.remove('collapsed');
  } else {
    // Collapse vocab pocket
    elements.diveInVocabPocketContent.classList.add('collapsed');
    elements.diveInVocabPocketToggle.classList.add('collapsed');
  }
}
...
// Webpage-specific vocabulary storage functions
async function addToWebpageVocabulary(term, url) {
  try {
    const result = await chrome.storage.local.get(['webpageVocabularies']);
    const webpageVocabularies = result.webpageVocabularies || {};
    
    if (!webpageVocabularies[url]) {
      webpageVocabularies[url] = [];
    }
    
    if (!webpageVocabularies[url].includes(term)) {
      webpageVocabularies[url].push(term);
      await chrome.storage.local.set({ webpageVocabularies });
      console.log(`📚 Added "${term}" to webpage vocabulary for "${url}"`);
    ...
    console.error('Error adding to webpage vocabulary:', error);
  }
}
...
async function addToVocabPocket(term) {
  try {
    ...
    // Also add to webpage vocabulary
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    const url = tab ? tab.url : '';
    if (url) {
      await addToWebpageVocabulary(term, url);
    }

    // Add to field vocabulary if user has a field selected
    if (userField) {
      await addToFieldVocabulary(userField, term);
    }
  } catch (error) {
...
async function removeFromVocabPocket(term) {
  try {
    ...
    // Remove from webpage vocabulary
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    const url = tab ? tab.url : '';
    if (url) {
      await removeFromWebpageVocabulary(term, url);
    }
    
    // Remove from field vocabulary if user has a field selected
    if (userField) {
...
async function getWebpageVocabulary(url) {
  try {
    const result = await chrome.storage.local.get(['webpageVocabularies']);
    const webpageVocabularies = result.webpageVocabularies || {};
    return webpageVocabularies[url] || [];
  } catch (error) {
    console.error('Error getting webpage vocabulary:', error);
    return [];
  }
}

async function removeFromWebpageVocabulary(term, url) {
  try {
    const result = await chrome.storage.local.get(['webpageVocabularies']);
    const webpageVocabularies = result.webpageVocabularies || {};
    
    if (webpageVocabularies[url]) {
      const index = webpageVocabularies[url].indexOf(term);
      if (index > -1) {
        webpageVocabularies[url].splice(index, 1);
        await chrome.storage.local.set({ webpageVocabularies });
        console.log(`📚 Removed "${term}" from webpage vocabulary for "${url}"`);
      }
    }
  } catch (error) {
    console.error('Error removing from webpage vocabulary:', error);
  }
}

async function getVocabPocket() {
  try {
    // Get main vocab pocket
    const result = await chrome.storage.local.get(['vocabPocket']);
    const vocabPocket = result.vocabPocket || [];
    
    // Also get webpage-specific vocabulary
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    const url = tab ? tab.url : '';
    const webpageVocab = url ? await getWebpageVocabulary(url) : [];
    
    // Return unique combination of both
    return [...new Set([...vocabPocket, ...webpageVocab])];
  } catch (error) {
...
// Field notes storage functions
async function getFieldNotes(field) {
  try {
    const result = await chrome.storage.local.get(['fieldNotes']);
    const fieldNotes = result.fieldNotes || {};
    return fieldNotes[field] || [];
  } catch (error) {
...
async function addToFieldNotes(field, note) {
  try {
    const result = await chrome.storage.local.get(['fieldNotes']);
    const fieldNotes = result.fieldNotes || {};
    ...
    fieldNotes[field].push(noteWithTimestamp);
    await chrome.storage.local.set({ fieldNotes });
    console.log(`📝 Added note to field notes for "${field}"`);
    return noteWithTimestamp;
  } catch (error) {
    console.error('Error adding to field notes:', error);
  }
}

async function removeFromFieldNotes(field, noteId) {
  try {
    const result = await chrome.storage.local.get(['fieldNotes']);
    const fieldNotes = result.fieldNotes || {};
    
    if (fieldNotes[field]) {
      const index = fieldNotes[field].findIndex(note => note.id === noteId);
      if (index > -1) {
        fieldNotes[field].splice(index, 1);
        await chrome.storage.local.set({ fieldNotes });
        console.log(`📝 Removed note from field notes for "${field}"`);
      }
    }
  } catch (error) {
    console.error('Error removing from field notes:', error);
  }
}
  ...
  try {
    ...
    if (fieldsWithHats.length === 0) {
      hatsGrid.innerHTML = `
        <div class="empty-state">
          ...
          <h3 class="empty-state-title">No hats collected yet</h3>
          <p class="empty-state-message">Select a field to get your first hat!</p>
        </div>
      `;
      return;
    }
    
    // Generate hat cards for each field
    ...
    document.querySelectorAll('.hat-card[data-field]').forEach(card => {
      card.addEventListener('click', async () => {
        ...
        // Show field vocabulary and notes in the hats page
        await showFieldVocabularyInHatsPage(field);
        await showFieldNotesInHatsPage(field);
      });
    });
...
// Show field notes in hats page
async function showFieldNotesInHatsPage(field) {
  try {
    console.log(`📝 Showing notes for field: ${field} in hats page`);
    
    const fieldNotes = await getFieldNotes(field);
    const notesGrid = document.querySelector('.notes-grid');
    
    if (!notesGrid) {
      console.error('Notes grid not found in hats page');
      return;
    }
    
    if (fieldNotes.length === 0) {
      notesGrid.innerHTML = `
        <div class="note-card empty-notes">
          <div class="empty-notes-content">
            <div class="empty-notes-icon">📝</div>
            <p class="empty-notes-text">No notes saved for ${field} yet</p>
            <button class="add-note-btn" onclick="addNewNoteForField('${field}')">Add Note</button>
          </div>
        </div>
      `;
      
      // Add event listener for empty state "Add Note" button
      const addNoteBtn = notesGrid.querySelector('.add-note-btn');
      if (addNoteBtn) {
        addNoteBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          addNewNoteForField(field);
        });
      }
    } else {
      // Create note cards for each note
      const notesHtml = fieldNotes.map(note => `
        <div class="note-card" data-note-id="${note.id}" data-field="${field}">
          <div class="note-title">${note.title}</div>
          <div class="note-content">${note.content}</div>
          <div class="note-date">${note.date}</div>
          <button class="delete-note-btn" data-note-id="${note.id}" data-field="${field}" title="Delete note">×</button>
        </div>
      `).join('');
      
      // Add "Add Note" button
      const addNoteHtml = `
        <div class="note-card add-note" onclick="addNewNoteForField('${field}')">
          <div class="add-note-text">Add Note</div>
        </div>
      `;
      
      notesGrid.innerHTML = notesHtml + addNoteHtml;
      ...
      notesGrid.querySelectorAll('.delete-note-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          ...
          console.log(`🗑️ Deleting note ${noteId} from ${field}`);
          
          // Remove from field notes
          await removeFromFieldNotes(field, noteId);
          ...
          // Update hat card notes count
          await loadHatsCollection();
          
          // If no more notes, show empty state
          if (notesGrid.children.length === 1) { // Only "Add Note" button left
            notesGrid.innerHTML = `
              <div class="note-card empty-notes">
                <div class="empty-notes-content">
                  <div class="empty-notes-icon">📝</div>
      ...
      // Add event listeners for "Add Note" cards
      notesGrid.querySelectorAll('.note-card.add-note').forEach(card => {
        card.addEventListener('click', () => {
          addNewNoteForField(field);
        });
      });
      
      // Add event listeners for "Add Note" buttons in empty state
      notesGrid.querySelectorAll('.add-note-btn').forEach(btn => {
    ...
    // Update the section title to show which field's notes are displayed
    const sectionTitle = document.querySelector('.notes-section .section-title');
    if (sectionTitle) {
      sectionTitle.textContent = `${field} Notes`;
    }
    
  } catch (error) {
    console.error('Error showing field notes in hats page:', error);
  }
}

// Add new note for a specific field
async function addNewNoteForField(field) {
  try {
    ...
    await addToFieldNotes(field, note);
    ...
    console.log(`📝 Added new note to ${field}: ${title}`);
  } catch (error) {
    console.error('Error adding new note for field:', error);
  }
}

// Make function globally accessible
window.addNewNoteForField = addNewNoteForField;

// Save sticker note to current field storage
async function saveStickerNoteToField(stickerNote) {
  try {
    // Get current user field
    const result = await chrome.storage.local.get(['userField']);
    const currentField = result.userField;
    
    if (!currentField) {
      console.log('📝 No field selected, skipping auto-save');
      return;
    }
    
    const title = stickerNote.querySelector('.sticker-title');
    const content = stickerNote.querySelector('.sticker-content p');
    ...
    // Skip if it's still the default placeholder
    if (noteContent === 'Click to edit your note...' || noteContent === 'Add your comments or reflections here...' || noteContent === '') {
      return;
    }
    
    // Check if this note is already saved (to avoid duplicates)
    const noteId = stickerNote.dataset.noteId;
    if (noteId) {
      console.log('📝 Note already saved, skipping duplicate');
      return;
    }
    ...
    const savedNote = await addToFieldNotes(currentField, note);
    
    // Mark this sticker note as saved
    if (savedNote) {
      stickerNote.dataset.noteId = savedNote.id;
      stickerNote.dataset.field = currentField;
      
      // Add visual indicator that it's saved
      const titleElement = stickerNote.querySelector('.sticker-title');
      if (titleElement && !titleElement.textContent.includes('✓')) {
        titleElement.textContent += ' ✓';
      }
    }
    
    console.log(`📝 Auto-saved sticker note to ${currentField}: ${noteTitle}`);
  } catch (error) {
    console.error('Error auto-saving sticker note to field:', error);
  }
}
...
function switchTab(activeButton) {
  console.log('🔄 switchTab called with button:', activeButton?.id);
  ...
  if (activeButton.id === 'btn-vocab-pocket') {
    console.log('🔄 switchTab: Showing dive-in results');
    elements.diveInResults.style.display = 'block';
  } else {
    console.log('🔄 switchTab: Hiding dive-in results');
...
async function handleVocabPocketClick() {
  ...
  // Update vocab pocket display (webpage-specific vocabulary)
  await updateVocabPocketDisplay();
  ...
  // Ensure all Dive In sections are visible when Dive In is opened
  if (elements.diveInTermExplanationContent) {
    elements.diveInTermExplanationContent.classList.remove('collapsed');
    elements.diveInTermExplanationToggle.classList.remove('collapsed');
  }
  
  if (elements.diveInVocabPocketContent) {
    elements.diveInVocabPocketContent.classList.remove('collapsed');
    elements.diveInVocabPocketToggle.classList.remove('collapsed');
  }
  ...
  if (elements.diveInAskHatrickContent) {
    ...
    elements.diveInAskHatrickToggle.classList.remove('collapsed');
  }
...
function handleDiveInToggle(sectionName) {
  ...
  if (sectionName === 'ask-hatrick') {
    ...
    header = elements.diveInNotesHeader;
    content = elements.diveInNotesContent;
    toggle = elements.diveInNotesToggle;
  } else {
  ...
  if (content && toggle) {
    ...
    // Use CSS classes like the page summary section
    if (isVisible) {
...
async function handleReadingMapClick() {
  try {
    ...
    if (summariesResponse && summariesResponse.ok && summariesResponse.summaries && Array.isArray(summariesResponse.summaries)) {
      // Build the structure map using DOM APIs to avoid inserting untrusted HTML
      elements.readingMapList.innerHTML = '';

      const wrapper = document.createElement('div');
      wrapper.className = 'structure-map-overview';

      // Legend (static HTML safe to set)
      const legendHtml = `
          ...
          <div class="legend-controls">
            <div class="legend-items">
              <div class="legend-item">
                ...
                <span class="legend-text">Critical Information</span>
              </div>
              <div class="legend-item">
                <span class="legend-color medium-importance"></span>
                <span class="legend-text">Helpful Context</span>
              </div>
              <div class="legend-item">
                <span class="legend-color low-importance"></span>
                <span class="legend-text">Background Info</span>
              </div>
            </div>
      ...
      flow.className = 'structure-flow';

      const summaries = summariesResponse.summaries;
      // Limit rendering to a reasonable number to avoid perf harm (e.g., 200 nodes) — but keep all if small
      const MAX_NODES = 200;
      const renderCount = Math.min(summaries.length, MAX_NODES);

      for (let index = 0; index < renderCount; index++) {
        const summary = summaries[index];
        const relevanceClass = (summary.relevanceLevel || 'low') + '-importance';

        // Generate title from the summary (ensures summarizer runs first)
        const sectionTitle = generateSectionTitle(summary.summary, index, summaries.length);

        const node = document.createElement('div');
        node.className = `structure-node ${relevanceClass}`;
        ...
        nodeHeader.setAttribute('data-node-index', String(index));
        nodeHeader.tabIndex = 0; // make focusable for accessibility

        const titleSpan = document.createElement('span');
        titleSpan.className = 'node-title';
        titleSpan.textContent = sectionTitle;

        nodeHeader.appendChild(titleSpan);

        const nodeSummary = document.createElement('div');
        nodeSummary.className = 'node-summary collapsible-content';
        nodeSummary.style.display = 'none';
        nodeSummary.textContent = summary.summary || '';

        // Toggle per-node on header click or Enter/Space
        const toggleSummary = () => {
          const isVisible = nodeSummary.style.display !== 'none';
          nodeSummary.style.display = isVisible ? 'none' : 'block';
        };
        nodeHeader.addEventListener('click', toggleSummary);
        nodeHeader.addEventListener('keypress', (e) => {
        ...
        nodeContent.appendChild(nodeSummary);
      ...
      // If we truncated nodes, show a simple notice
      if (summaries.length > renderCount) {
        const moreNotice = document.createElement('div');
        moreNotice.className = 'muted';
        moreNotice.textContent = `Showing ${renderCount} of ${summaries.length} sections. Refresh or narrow selection to see more.`;
        wrapper.appendChild(moreNotice);
      }

      wrapper.appendChild(flow);
      elements.readingMapList.appendChild(wrapper);

      // Add global toggle functionality (works with .collapsible-content elements)
...
// Removed term explanation toggle event listener since toggle button was removed
elements.diveInVocabPocketHeader.addEventListener('click', handleVocabPocketToggle);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  switch (message.type) {
    ...
    default:
      // Allow other listeners to handle this message
      console.log('📚 First listener: Passing unhandled message to other listeners:', message.type || message.action);
      return true;
  }
});
...
function displayExplanation(explanationData) {
  ...
  chrome.storage.local.get(['lastSelection'], (result) => {
    ...
    if (explanationSection) {
      ...
      // Determine which explanation to show based on user experience level
      const showBeginner = userExperience === 'beginner';
      const explanationToShow = showBeginner ? explanationData.beginner : explanationData.advanced;
      
      // Update explanation content - show only the appropriate explanation
      ...
      if (beginnerBox && advancedBox) {
        ...
        // Show only the appropriate explanation
        if (showBeginner) {
          beginnerBox.innerHTML = `<p>${formatExplanation(explanationToShow)}</p>`;
          beginnerBox.style.display = 'block';
        } else {
          advancedBox.innerHTML = `<p>${formatExplanation(explanationToShow)}</p>`;
          advancedBox.style.display = 'block';
        }
      }
      
      // Hide the tabs since we're not using them anymore
      const tabs = explanationSection.querySelectorAll('.explanation-tab');
      tabs.forEach(tab => {
        tab.style.display = 'none';
      });
      
      // Hide the save button since we're using only the heart icon
...
function generateSectionTitle(summary, index, totalSections) {
  // Create a safe plain-text version of the summary
  const text = (summary || '').replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
  if (!text) return `Section ${index + 1}`;

  // Common leading stopwords to skip when forming a short title
  const stopwords = new Set(['the','a','an','this','that','these','those','in','of','on','and','for','to','with','by','from','is','are','as','it','its','we','our','paragraph','section','note']);

  // Split into words, strip punctuation from ends
  const rawWords = text.split(/\s+/).map(w => w.replace(/^[^a-zA-Z0-9]+|[^a-zA-Z0-9]+$/g, '')).filter(Boolean);

  // Build up to 6 meaningful words, skipping an initial stopword if present
  const titleWords = [];
  for (let i = 0; i < rawWords.length && titleWords.length < 6; i++) {
    const w = rawWords[i];
    if (titleWords.length === 0 && stopwords.has(w.toLowerCase())) {
      continue;
    }
    titleWords.push(w);
  }

  // Fallback: if nothing collected, just take the first up to 6 raw words
  if (titleWords.length === 0) {
    titleWords.push(...rawWords.slice(0, 6));
  }

  // Capitalize each word
  const capped = titleWords.map(w => w.charAt(0).toUpperCase() + w.slice(1));

  // Append ellipsis when truncated
  const truncated = rawWords.length > capped.length ? '…' : '';

  return (capped.join(' ') || `Section ${index + 1}`) + truncated;
}
...
// Handle term explanation display (simplified - no API calls)
async function displayTermExplanation(term) {
  ...
  // Switch to dive in tab first to avoid flickering
  console.log('📚 Panel: Switching to Dive In tab');
  switchTab(elements.btnVocabPocket);
  
  // Show dive in section if not already visible
  elements.diveInResults.style.display = 'block';
  
  // Store the term being explained
  elements.selectedTermText.textContent = term;
  // Ensure the selected term header (with heart) is visible
  if (elements.selectedTermDisplay) {
    elements.selectedTermDisplay.style.display = 'block';
  }
  
  // Show loading placeholder with just the explanation (no duplicate term card)
  elements.termExplanationResults.innerHTML = `
    <div class="keyword-explanation visible" id="explanation-term">
  ...
  // Set up heart button functionality for the main term display (not the duplicate)
  const mainHeartBtn = elements.heartButton;

  // Check if term is already in vocabulary and update the main heart button
  const vocabPocket = await getVocabPocket();
  if (mainHeartBtn) {
    const heartIcon = mainHeartBtn.querySelector('.heart-icon');
    if (vocabPocket.includes(term)) {
      mainHeartBtn.classList.add('saved');
      if (heartIcon) heartIcon.textContent = '♥';
    } else {
      mainHeartBtn.classList.remove('saved');
      if (heartIcon) heartIcon.textContent = '♡';
    }
  }

  // Ensure all dive in sections stay expanded and visible
  if (elements.diveInTermExplanationContent) {
    elements.diveInTermExplanationContent.style.display = 'block';
    elements.diveInTermExplanationContent.classList.remove('collapsed');
  }
  
  if (elements.diveInTermExplanationHeader) {
    elements.diveInTermExplanationHeader.classList.add('expanded');
  }
  
  if (elements.diveInVocabPocketContent) {
    elements.diveInVocabPocketContent.style.display = 'block';
    elements.diveInVocabPocketContent.classList.remove('collapsed');
  }
  
  if (elements.diveInVocabPocketHeader) {
    elements.diveInVocabPocketHeader.classList.add('expanded');
  }
  
  // Call the real API to get explanation
  try {
    // Get page context and user settings for better explanation
    const result = await chrome.storage.local.get(['userField', 'userExperience', 'pageText']);
    const field = result.userField || '';
    const experience = result.userExperience || '';
    const pageContext = result.pageText || '';
    ...
    if (response && response.success) {
      // Store explanation in storage for persistence
      ...
      handleTermExplanationResult(response);
    } else {
      throw new Error('Failed to get explanation');
    }
  } catch (error) {
    console.error('Error getting term explanation:', error);
    // Instead of showing error box, keep the loading message
    const explanationElement = document.getElementById('explanation-term');
  ...
  // Add click handler for the term card (same as keywords)
  addTermCardHandler();
  
  console.log('📚 Panel: Term explanation displayed successfully');
}

// Get term explanation using Chrome Prompt API
async function getTermExplanation(term) {
  console.log('📚 Panel: Getting explanation for term:', term);
  
  try {
    // Check if chrome.ai is available
    if (!chrome.ai || !chrome.ai.languageModel) {
      console.log('📚 Panel: Chrome AI API not available, showing fallback explanation');
      showFallbackExplanation(term);
      return;
    }
    
    // Check if the language model is available
    const availability = await chrome.ai.languageModel.availability();
    console.log('📚 Panel: Language model availability:', availability);
    
    if (availability === 'unavailable') {
      throw new Error('Language model is not available on this device. Please check if you have sufficient storage (22GB+) and compatible hardware.');
    }
    
    // Create a session
    const session = await chrome.ai.languageModel.create();
    console.log('📚 Panel: Language model session created');
    
    // Create a simple prompt
    const prompt = `Explain the term "${term}" in simple terms within 100 words. Make it easy to understand for someone learning about this topic.`;
    
    console.log('📚 Panel: Sending prompt to language model');
    
    // Get the explanation
    const explanation = await session.prompt(prompt);
    console.log('📚 Panel: Received explanation:', explanation);
    
    // Display the explanation using the same pattern as keywords
    const explanationElement = document.getElementById('explanation-term');
    if (explanationElement) {
    ...
    console.log('📚 Panel: Explanation displayed successfully');
    
    // Clean up the session
    session.destroy();
    
  } catch (error) {
    console.error('📚 Panel: Error getting term explanation:', error);
...
// Show fallback explanation when AI API is not available
function showFallbackExplanation(term) {
  console.log('📚 Panel: Showing fallback explanation for:', term);
  
  // Simple fallback explanation
  const fallbackExplanation = `"${term}" is a technical term that appears in this context. While we'd normally provide a detailed AI-generated explanation, the Chrome AI features are not currently available on your device. This could be due to Chrome version requirements (Chrome 138+ needed) or hardware limitations (22GB+ storage, 4GB+ VRAM or 16GB+ RAM required).`;
  
  // Display the fallback explanation using the same pattern as keywords
  const explanationElement = document.getElementById('explanation-term');
  if (explanationElement) {
    explanationElement.innerHTML = formatExplanation(fallbackExplanation);
    explanationElement.classList.add('visible');
  }
}

// Add click handler for term card (same as keywords)
function addTermCardHandler() {
  const termCard = document.querySelector('.keyword-card[data-keyword-id="term"]');
  if (termCard) {
    termCard.addEventListener('click', function(e) {
      ...
      const explanation = document.getElementById('explanation-term');
      const isVisible = explanation.classList.contains('visible');
      
      // Close all other explanations first (like keywords do)
      document.querySelectorAll('.keyword-explanation.visible').forEach(exp => {
        exp.classList.remove('visible');
      });
      document.querySelectorAll('.keyword-card.active').forEach(c => {
...
// Handle term explanation results from background
function handleTermExplanationResult(result) {
  console.log('📚 Panel: Received term explanation result:', result);
  
  // Make sure the term explanation section is visible
  if (elements.diveInTermExplanationContent) {
    elements.diveInTermExplanationContent.style.display = 'block';
    elements.diveInTermExplanationContent.classList.remove('collapsed');
  }
  
  if (result.success && (result.beginner || result.advanced)) {
    console.log('📚 Panel: Term explanation successful');
    
    // Determine which explanation to show based on user experience level
    const showBeginner = userExperience === 'beginner';
    const explanationToShow = showBeginner ? result.beginner : result.advanced;
    
    console.log('📚 Panel: Showing explanation for user level:', userExperience);
    
    // Create explanation HTML with visible state built-in (no duplicate term card)
    elements.termExplanationResults.innerHTML = `
      <div class="keyword-explanation visible" id="explanation-term">
        <p>${formatExplanation(explanationToShow)}</p>
      </div>
    `;
    ...
    console.error('📚 Panel: Term explanation failed:', result.error);
    
    // Show error state without duplicate term card
    elements.termExplanationResults.innerHTML = `
      <div class="explanation-error">
        <p class="muted">Sorry, couldn't get explanation for "${result.term}". Please try again.</p>
        <p class="muted" style="font-size: 11px; color: #999;">Error: ${result.error}</p>
        <button class="btn btn-secondary" onclick="displayTermExplanation('${result.term}')">Retry</button>
      </div>
    `;
  }
}
...
// Update vocab pocket display (both webpage and field vocabularies)
async function updateVocabPocketDisplay() {
  try {
    // Get current URL for webpage-specific vocabulary
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    const url = tab ? tab.url : '';
    
    // Get both vocabularies
    const webpageVocab = url ? await getWebpageVocabulary(url) : [];
    const fieldVocab = userField ? await getFieldVocabulary(userField) : [];
    
    // Combine vocabularies and remove duplicates
    const allTerms = [...new Set([...webpageVocab, ...fieldVocab])];
    
    if (allTerms.length === 0) {
      elements.diveInVocabPocketContent.innerHTML = `
        <p class="muted">No vocabulary terms saved yet</p>
      `;
    } else {
      // Create keyword cards with solid hearts for saved terms
      const keywordCards = allTerms.map(term => `
        <div class="core-keyword-item">
          <div class="keyword-card" data-term="${term}">
            <span class="keyword-term">${term}</span>
            <button class="heart-btn saved" data-term="${term}" title="Remove from vocabulary">
              <span class="heart-icon">♥</span>
            </button>
          </div>
        </div>
      ...
      // Add click handlers for vocab pocket terms
      addVocabPocketClickHandlers();
      
      // Ensure all heart buttons are solid in the vocab pocket
      elements.diveInVocabPocketContent.querySelectorAll('.heart-btn').forEach(btn => {
        btn.classList.add('saved');
        const heartIcon = btn.querySelector('.heart-icon');
        if (heartIcon) {
          heartIcon.textContent = '♥';
    ...
    console.error('❌ Error updating vocab pocket display:', error);
  }
}

// Update field vocabulary display (for hats collection)
...
// Add click handlers for vocab pocket terms
function addVocabPocketClickHandlers() {
  const keywordCards = elements.diveInVocabPocketContent.querySelectorAll('.keyword-card');
  ...
  keywordCards.forEach(card => {
    card.addEventListener('click', (e) => {
      ...
      console.log('📚 Vocab pocket term clicked:', term);
  ...
  // Add heart button click handlers for vocab pocket
  const heartButtons = elements.diveInVocabPocketContent.querySelectorAll('.heart-btn');
  ...
  heartButtons.forEach(btn => {
    btn.addEventListener('click', async function(e) {
      ...
      // Remove from both vocab pocket and webpage vocabulary
      await removeFromVocabPocket(term);
      this.classList.remove('saved');
      heartIcon.textContent = '♡';
      console.log(`💔 Removed "${term}" from vocab pocket`);
...
function setupStickerNote(note) {
  ...
  content.addEventListener('click', () => {
    if (content.textContent === 'Click to edit your note...' || content.textContent === 'Add your comments or reflections here...') {
      content.textContent = '';
      content.focus();
    }
  });
  ...
  content.addEventListener('blur', async () => {
    if (content.textContent.trim() === '') {
      // Use appropriate placeholder based on whether this is a note with selected text
      const title = note.querySelector('.sticker-title');
      const hasSelectedText = title && title.textContent.startsWith('"') && title.textContent.endsWith('"');
      const placeholder = hasSelectedText ? 'Add your comments or reflections here...' : 'Click to edit your note...';
      content.textContent = content.dataset.placeholder || placeholder;
    } else {
      // Auto-save sticker note to current field
      await saveStickerNoteToField(note);
    }
  });
  ...
  closeBtn.addEventListener('click', async (e) => {
    ...
    // Remove from field storage if it was saved
    const noteId = note.dataset.noteId;
    const field = note.dataset.field;
    if (noteId && field) {
      await removeFromFieldNotes(field, noteId);
      console.log(`📝 Removed note from field storage: ${noteId}`);
    }
...
let lastAddToNotesTime = 0;
function addTextToNotes(text) {
  const now = Date.now();
  // Prevent duplicate calls within 500ms
  if (now - lastAddToNotesTime < 500) {
    console.log('📝 Duplicate addTextToNotes call prevented');
    return;
  }
  lastAddToNotesTime = now;
  
  console.log('📝 addTextToNotes called with text:', text);
  const container = document.querySelector('.sticker-notes-container');
  if (!container) {
    console.log('📝 No notes container found');
    return;
  }
  ...
  console.log('📝 Found existing notes:', existingNotes.length);
  let blankNote = null;
  ...
  if (blankNote) {
    ...
    if (content) content.textContent = 'Add your comments or reflections here...';
    
    // Auto-save to field storage
    setTimeout(() => saveStickerNoteToField(blankNote), 100);
    ...
    newNote.innerHTML = `
      ...
      <div class="sticker-content">
        <p>Add your comments or reflections here...</p>
      </div>
    `;
    ...
    // Auto-save to field storage
    setTimeout(() => saveStickerNoteToField(newNote), 100);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  console.log('📚 Panel: Message sender:', sender);
  console.log('📚 Panel: Message type/action:', message.type || message.action);
  console.log('📚 Panel: Panel script is listening for messages');
  ...
  } else if (message.type === 'showTermExplanation') {
    console.log('📚 Panel: Showing term explanation for:', message.term);
    console.log('📚 Panel: Full message received:', message);
    // Simple test - just show the selected word
    displayTermExplanation(message.term);
  } else {
...
chrome.storage.onChanged.addListener((changes, namespace) => {
  ...
  // Handle term explanation requests (old method - keeping for compatibility)
  if (changes.termExplanationRequest) {
    const request = changes.termExplanationRequest.newValue;
    console.log('📚 Panel: Term explanation request from storage:', request);
    if (request && request.action === 'displayTermExplanation') {
      console.log('📚 Panel: Calling displayTermExplanation from storage with term:', request.term);
      displayTermExplanation(request.term);
    }
  }
  
  // Handle term explanation results (new method)
  if (changes.termExplanationResult) {
    const result = changes.termExplanationResult.newValue;
    console.log('📚 Panel: Term explanation result from storage:', result);
    if (result && result.term) {
      console.log('📚 Panel: Processing term explanation result for term:', result.term);
      handleTermExplanationResult(result);
    }
  }
});

// Check for pending term explanation requests/results when panel loads
chrome.storage.local.get(['termExplanationRequest', 'termExplanationResult'], (result) => {
  // Handle pending requests (old method)
  if (result.termExplanationRequest) {
    console.log('📚 Panel: Found pending term explanation request:', result.termExplanationRequest);
    if (result.termExplanationRequest.action === 'displayTermExplanation') {
      console.log('📚 Panel: Processing pending term explanation request');
      displayTermExplanation(result.termExplanationRequest.term);
      // Clear the request after processing
      chrome.storage.local.remove(['termExplanationRequest']);
    }
  }
  
  // Handle pending results (new method)
  if (result.termExplanationResult) {
    console.log('📚 Panel: Found pending term explanation result:', result.termExplanationResult);
    if (result.termExplanationResult.term) {
      console.log('📚 Panel: Processing pending term explanation result');
      handleTermExplanationResult(result.termExplanationResult);
      // Clear the result after processing
      chrome.storage.local.remove(['termExplanationResult']);
    }
  }
});
